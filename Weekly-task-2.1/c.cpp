...
class HashTable {
 public:
  ...
  bool IsInTheTable(string string) {
    ...
    for (size_t i = 0; i < size_; i++) {
      ...
      if (table_[position].occupied == true && table_[position].value == string) {
        return true;
      }
    }
    return false;
 ...
 private:
  ...
  void Resize() {
    size_t new_size = SieveOfEratosthenes(size_ * 2);
    Cell *new_table = new Cell[new_size];
    for (size_t i = 0; i < size_; i++) {
      string string_to_be_copied = table_[i].value;
      if (table_[i].occupied == true && string_to_be_copied != "") {
  ...
  void Add(Cell *table, string string, size_t size) {
    ...
    for (size_t i = 0; i < size_; i++) {
      ...
      if (table[position].occupied == false) {
