...
void DFS(int start, vector<vector<Edge>> &graph, vector<bool> &used, vector<int> &depths, vector<int> &back_depths,
         priority_queue<int, vector<int>, std::greater<int>> &bridges, int p = -1) {
  used[start] = true;
  ...
  for (Edge u : graph[start]) {
    if (u.to != p) {
      if (used[u.to]) {
        ...
        DFS(u.to, graph, used, depths, back_depths, bridges, start);
        depths[start] = min(depths[start], depths[u.to]);
        if (back_depths[start] < depths[u.to]) {
          bridges.push(u.index);
        }
      }
    }
  }
...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    if (!used[i]) {
      DFS(i, graph_list, used, depths, back_depths, bridges);
    }
  }
