...
queue<Point> FindConvexHullByJarvis(const vector<Point> &points, 
                            const Point &zero_point = {std::numeric_limits<int>::max(), std::numeric_limits<int>::max()})
{
  queue<Point> hull;
  Point from = zero_point;
  hull.push(zero_point);
  while (true) {
    Point current = zero_point;
    for (Point p : points) {
      Vector to_current(from, current);
      Vector to_p(from, p);
      int angle_between_new_and_current = (to_p ^ to_current);

      if (angle_between_new_and_current < 0 ||
          (angle_between_new_and_current == 0 && to_p.LengthSquared() > to_current.LengthSquared())) {
        current = p;
      }
...
{
  queue<Point> hull = FindConvexHullByJarvis(points, starting_point);
  cout << hull.size() << '\n';
