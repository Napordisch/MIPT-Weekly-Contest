...
bool RayBetweenPointSegments(const Ray &r, const Point &p, const Segment &left_segment, const Segment &right_segment, bool point_on_ray = false) 
{
	if (!point_on_ray)
	{
...
bool PointInsidePolygon(const Point &p, Polygon &s) 
{
	...
	for (size_t i = 0; i < s.points.size(); ++i)
	{
		...
		if (point_on_ray) 
		{
			...
			if(RayBetweenPointSegments(horizontal_from_point, polygon_point, left_segment, right_segment,point_on_ray))
			{
				++amount_of_intersections;
			}
		}
	...
	cout << "amount of intersections: "<< amount_of_intersections << '\n';
